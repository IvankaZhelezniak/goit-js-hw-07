...
listItemRef.map(item => {
    const titleRef = item.querySelector('h2');
    const itemCategoriesRef = item.querySelectorAll('ul li');
    return {
        titleRef: titleRef.textContent,
         itemCategoriesRef: itemCategoriesRef.length
    }
}).forEach(entry => {
    console.log(`Категория: `, entry.titleRef, `\r\nКоличество элементов: `, entry.itemCategoriesRef);
